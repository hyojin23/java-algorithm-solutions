...
class Point_1st {
    ...
    Point_1st(int x, int y, int breakWallCnt, int dis) {
        this.x = x;
        this.y = y;
...
public class Main_1st {
    ...
    public int BFS(int x, int y, int breakWallCnt, int dis) {
        ...
        q.offer(new Point_1st(x, y, breakWallCnt, dis));
        int[] dx = {-1, 0, 1, 0};
        int[] dy = {0, 1, 0, -1};
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < N && ny >= 0 && ny < M && !visited[nx][ny][cur.breakWallCnt]) {
                    if (board[nx][ny] == 0) {
                        ...
                        q.offer(new Point_1st(nx, ny, cur.breakWallCnt, cur.dis + 1));
                    }
                    else if (cur.breakWallCnt == 0) {
                        visited[nx][ny][cur.breakWallCnt] = true;
                        int newCnt = cur.breakWallCnt + 1;
                        q.offer(new Point_1st(nx, ny, newCnt, cur.dis + 1));
                    }
                }
            }
        }
